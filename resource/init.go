...
type DBItf interface {
	...
	CreateRoom(roomName string, adminID string, description string, categoryID string) (model.Room, error)
...
func (dbr *DBResource) CreateRoom(roomName string, adminID string, description string, categoryID string) (model.Room, error) {
	query := `
		INSERT INTO
			room
		(
...
func (dbr *DBResource) EditGroupchat(name, description, categoryID string) (model.Room, error) {
	query := `
		UPDATE
			room
		SET
	...
	_, err := dbr.db.Exec(query, name, description, categoryID)
	if err != nil {
		return model.Room{}, err
	}
...
func (dbr *DBResource) AddRoomParticipant(roomID, userID string) error {
	query := `
		INSERT INTO
			room_participant
		(
